...
//		logStep("Returned From 'Navigate to Privacy'");
	...
	public void select_Airlock_UserGroup(String userGroupName) throws Exception {
		...
		if (userGroupName.equalsIgnoreCase("Clear") || userGroupName.equalsIgnoreCase("UnSelect")) {
			...
			} else if (userGroupName.equalsIgnoreCase("Criteo")) {
				/**
				 * To get amazon preload amazon call 'amazon' usergroup to be enabled as per
				 * https://jira.weather.com:8443/browse/IOSFLAG-7375
				 * This usergroup selection to be removed later, once config moved to production
	...
	@Step("Set GPS Spoof")
	public void GPS_Spoof(String LocationName) throws Exception {
		ReadExcelValues.excelValues("Smoke", "TestMode");
		...
		WebElement SpoofSwitch = Ad.findElementByClassName("XCUIElementTypeSwitch");
		String SpoofValue = SpoofSwitch.getAttribute("value");
		if (SpoofValue.equals("1")) {
			System.out.println("GPS Spoofing is already enabled");
		} else {
			SpoofSwitch.click();
			WebElement addressTextBox = Ad.findElementByClassName("XCUIElementTypeTextField");
			addressTextBox.clear();
			addressTextBox.sendKeys(LocationName);
			Ad.findElementByName("Lookup Address or Lat/Lon").click();
			Thread.sleep(5000);
		}
